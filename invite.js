...
const weddingDate = new Date('2026-11-27T17:00:00-06:00');
const planningStart = new Date('2026-08-01T00:00:00-06:00');
...
// Reloj de arena: la arena superior baja y el monticulo inferior crece
// segun el progreso real entre el inicio de planes y la boda.
...
const SAND_TOP_FULL_Y = 44;   // superficie con el bulbo lleno
const SAND_TOP_EMPTY_Y = 158; // superficie al nivel del cuello
...
function updateCountdown() {
  ...
  if (countdownRing) {
    ...
    const progress = totalRange > 0 ? currentRange / totalRange : 0;

    if (sandTop) {
      const surfaceY = SAND_TOP_FULL_Y + (SAND_TOP_EMPTY_Y - SAND_TOP_FULL_Y) * progress;
      sandTop.setAttribute('transform', `translate(0 ${surfaceY.toFixed(2)})`);
    }

    if (sandBottom) {
      const moundScale = Math.max(0.08, progress).toFixed(3);
      sandBottom.setAttribute('transform', `translate(0 298) scale(1 ${moundScale}) translate(0 -298)`);
    }

    if (sandStream) {
      sandStream.style.opacity = progress >= 1 ? '0' : '1';
    }
  }
}
...
function enforceDeadlineRules() {
  ...
  setRsvpStatus('El plazo para confirmar asistencia cerro el 15 de septiembre de 2026.', 'error');
...
/* ============================================================
   Asistente por pasos del RSVP: una pregunta a la vez.
   Orden: 0 ¿asistes? -> 1 quien eres -> 2 acompanantes (solo si
   asiste) -> 3 opcional + confirmar.
   ============================================================ */
...
function goToWizardStep(stepId) {
  ...
  wizardSteps.forEach((step) => {
    step.classList.toggle('is-active', Number(step.dataset.wstep) === stepId);
  });
  ...
  if (skippedBar) {
    skippedBar.style.display = order.includes(2) ? '' : 'none';
  }

  if (wizardHint) {
    wizardHint.textContent = `Paso ${pos + 1} de ${order.length}`;
  }
...
function validateWizardStep(stepId) {
  if (stepId === 0) {
    const selected = document.querySelector('input[name="attend"]:checked');
    return selected ? '' : 'Selecciona si asistiras o no.';
  }

  if (stepId === 1) {
    if (!fullNameInput || fullNameInput.value.trim().length < 5) {
...
if (wizardNext) {
  wizardNext.addEventListener('click', () => {
    ...
    if (error) {
      setRsvpStatus(error, 'error');
      return;
    }
    const order = wizardOrder();
    const pos = order.indexOf(wizardStepId);
    if (pos < order.length - 1) {
      goToWizardStep(order[pos + 1]);
    }
  });
}
...
// Elegir si/no avanza solo, tras un instante para ver la seleccion.
attendInputs.forEach((input) => {
  input.addEventListener('change', () => {
    if (wizardStepId === 0) {
      setTimeout(() => goToWizardStep(1), 280);
    }
  });
});

// Al editar una respuesta guardada, el asistente vuelve al inicio.
if (rsvpEditButton) {
  rsvpEditButton.addEventListener('click', () => goToWizardStep(0));
}
...
function parseRsvpPayload(formData) {
  ...
  // Ya no se pide grupo: cada confirmacion agrupa bajo quien confirma.
  const groupName = (formData.get('groupName') || '').toString().trim() || fullName;
...
// Hojas de otono cayendo: pocos elementos, animaciones solo de transform (composited).
...
const LEAF_SOURCES = ['hoja-1.png', 'hoja-2.png', 'hoja-3.png'];

function buildPetals() {
  if (!petalLayer) return;
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  // Pocas hojas, caida suave: vaiven original con densidad baja
  // y hojas semitransparentes.
  const petalCount = 5;
  ...
  for (let i = 0; i < petalCount; i += 1) {
    ...
    petal.style.setProperty('--px', `${(i / petalCount) * 100 + Math.random() * 12}vw`);
    petal.style.setProperty('--size', `${22 + Math.random() * 14}px`);
    petal.style.setProperty('--petal-alpha', (0.35 + Math.random() * 0.2).toFixed(2));
    petal.style.setProperty('--fall-duration', `${13 + Math.random() * 9}s`);
    petal.style.setProperty('--fall-delay', `${-Math.random() * 20}s`);
    // Viento constante: todas derivan suave hacia la derecha, apenas
    // inclinadas en la direccion del viento (sin giros completos).
    petal.style.setProperty('--drift', `${(5 + Math.random() * 7).toFixed(1)}vw`);
    petal.style.setProperty('--spin-from', `${Math.round(4 + Math.random() * 10)}deg`);
    petal.style.setProperty('--spin-to', `${Math.round(10 + Math.random() * 14)}deg`);
    ...
    petalImg.src = LEAF_SOURCES[i % LEAF_SOURCES.length];
    petalImg.alt = '';
...
/* ============================================================
   Save the Date: fundido a luz (transicion de cine).
   Coreografia por scroll en tres actos:
   - t 0.00-0.55  inmersion: push-in lento + vineta calida.
   - t 0.35-0.70  fundido: la marea de luz dorada cubre todo y el
                  sonido del trailer se apaga en paralelo.
   - t ~0.70      con la pantalla cubierta de luz, el video se muda
                  al arco del hero (nadie ve el corte) y el hero
                  libera su entrada en cascada; City of Stars entra
                  en fade desde la portada.
   - t 0.72-0.95  la luz se levanta y revela la invitacion.
   Todo es opacidad + transform en un loop rAF: fluido a 120 Hz.
   ============================================================ */
...
if (videoIntro && videoFrame && introVideo && heroPortrait) {
  ...
  const smooth = (x) => x * x * (3 - 2 * x); // suavizado clasico
  let trackLength = 1;
  let docked = false;
  ...
  function measure() {
    trackLength = Math.max(1, videoIntro.offsetHeight - window.innerHeight);
  }
  ...
  function parentSong(method) {
    try {
      if (window.parent && window.parent !== window && typeof window.parent[method] === 'function') {
        window.parent[method]();
      }
    } catch (error) { /* invitacion abierta sin portada */ }
  }
  ...
  function dock() {
    ...
    heroPortrait.classList.add('has-video');
    heroPortrait.appendChild(introVideo);
    videoFrame.style.display = 'none';
    if (ambientVideo) ambientVideo.pause();
    safePlay(introVideo);
    releaseHero();
    parentSong('fadeInSong');
  }

  function undock() {
    if (!docked) return;
    docked = false;
    heroPortrait.classList.remove('has-video');
    videoFrame.insertBefore(introVideo, videoFrame.firstChild);
    videoFrame.style.display = '';
    // De vuelta al trailer: su sonido regresa (el volumen lo gobierna
    // applyAtmosphere segun el scroll) y la cancion se retira suave.
    if (soundUnlocked) {
      introVideo.muted = false;
    }
    parentSong('fadeOutSong');
    if (ambientVideo && window.matchMedia('(min-width: 700px)').matches) {
      safePlay(ambientVideo);
    }
  }

  function washOpacity(t) {
    // Sube 0.35 -> 0.70, se mantiene llena, baja 0.72 -> 0.95.
    if (t < 0.7) return smooth(clamp01((t - 0.35) / 0.35));
  ...
  function frameStep() {
    ...
    // Acto 3: con la pantalla cubierta, el corte invisible.
    if (shownT >= 0.7 && wash > 0.92) {
      dock();
    } else if (docked && shownT < 0.6) {
      undock();
    }
  ...
  function cinematicScrollTo(top, duration) {
    ...
    function step(now) {
      ...
      window.scrollTo({ top: from + delta * easeInOut(k), behavior: 'instant' });
      if (k < 1) requestAnimationFrame(step);
    }
    requestAnimationFrame(step);
  }

  // Arranque del trailer con sonido. La portada lo invoca dentro del
  ...
  if (videoSoundButton) {
    videoSoundButton.addEventListener('click', () => {
      ...
      safePlay(introVideo);
      videoSoundButton.hidden = true;
    });
  }

  // Al terminar el trailer: bucle silencioso y, si nadie ha scrolleado,
  // el auto-scroll lleva la escena hasta la revelacion.
  introVideo.addEventListener('ended', () => {
    introVideo.muted = true;
    introVideo.loop = true;
    safePlay(introVideo);
    if (!docked && window.scrollY < 60) {
      cinematicScrollTo(trackLength + 2, 3200);
    }
  });

  // Fondo ambiental sincronizado (deriva > 0.4s).
  if (ambientVideo) {
    setInterval(() => {
      if (docked || getComputedStyle(ambientVideo).display === 'none') return;
      if (Math.abs(ambientVideo.currentTime - introVideo.currentTime) > 0.4) {
        try { ambientVideo.currentTime = introVideo.currentTime; } catch (error) { /* buffering */ }
      }
    }, 4000);
  }
  ...
  // Fuera de pantalla, el video acoplado se pausa (bateria y datos).
  const introVisibility = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
  ...
  introVisibility.observe(introVideo);
...
/* ============================================================
   Momentos interactivos de la invitacion
   - Boletos de la agenda: toque para voltear (links intactos).
   - Campana de la cena: toque para levantar y revelar el menu.
   - Tarjeta Sinpe: inclinacion 3D con brillo que sigue el dedo.
   - La promesa: la frase se escribe palabra por palabra.
   ============================================================ */
...
document.querySelectorAll('[data-ticket]').forEach((ticket) => {
  ticket.addEventListener('click', (event) => {
    ...
    ticket.classList.add('was-flipped');
    ticket.classList.toggle('is-flipped');
  });
});

// Campana del menu
const clocheButton = document.getElementById('cloche');
if (clocheButton) {
  const revealMenu = () => {
    const panel = clocheButton.closest('.aporte__panel');
    if (panel) panel.classList.add('menu-revealed');
  };
  clocheButton.addEventListener('click', revealMenu);
  clocheButton.addEventListener('keydown', (event) => {
...
// Tarjeta Sinpe con inclinacion y brillo
const tiltWrap = document.getElementById('sinpe-tilt');
if (tiltWrap) {
  const tiltCard = tiltWrap.querySelector('.sinpe-card');
  ...
  tiltWrap.addEventListener('pointermove', (event) => {
    ...
    const rx = (0.5 - y) * 12;
    const ry = (x - 0.5) * 14;
    tiltCard.style.transition = 'transform 0.08s linear';
  ...
  tiltWrap.addEventListener('pointerleave', resetTilt);
  tiltWrap.addEventListener('pointerup', resetTilt);
}

// La promesa, palabra por palabra
const quoteLine = document.querySelector('.quote__line');
if (quoteLine) {
  const words = quoteLine.textContent.trim().split(/\s+/);
  quoteLine.textContent = '';
  words.forEach((word, index) => {
    const span = document.createElement('span');
    span.className = 'quote__w';
    span.style.setProperty('--wi', String(index));
    span.textContent = word;
    quoteLine.appendChild(span);
    if (index < words.length - 1) {
      quoteLine.appendChild(document.createTextNode(' '));
    }
  });
}
